...
import { ref } from 'vue'
import EvalChecklist from '@/components/evaluate/EvalChecklist.vue'
import Speech from '@/components/Speech.vue'
import EvalComments from '@/components/evaluate/EvalComments.vue'

const props = defineProps<{ list: Checklist }>()
...
q-dialog(v-model="modelValue")
	q-card.maindialog
		...
		q-card-section
			.text-h6 {{ props.list.list }}
			.total
				div {{ props.list.date }} - 13:05
				.text-weight-bold Общая оценка - {{ props.list.mark }}
			...
			q-tabs(v-model="tab" dense active-color="primary" indicator-color="primary" align="left" )
				q-tab(name="anketa" label="Чеклист")
			...
			q-tab-panels(v-model="tab" animated)
				q-tab-panel(name="anketa")
					q-scroll-area
						EvalChecklist(:mark='props.list.mark')
